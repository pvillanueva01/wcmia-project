<?php
/*
Plugin Name: WCMIA Notification Bar
...
function wnb_general_settings_page() {
	add_submenu_page(
		'options-general.php',
		__( 'WCMIA Notifications Bar', 'wcmia-notifications' ),
		__( 'Notifications Bar', 'wcmia-notifications' ),
		'manage_options',
		'wcmia_notifications',
...
function wnb_render_settings_page() {
	...
	<div class="wrap">
		...
		<h2><?php esc_html_e( 'WCMIA Notification Bar Settings', 'wcmia-notifications' ); ?></h2>
...
function wnb_initialize_settings() {
	add_settings_section(
		...
		__( 'General Settings', 'wcmia-notifications' ),    // Title to be displayed on the administration page
	...
	add_settings_field(
		...
		__( 'Notification Text', 'wcmia-notifications' ),   // Title to be displayed for this field
	...
	add_settings_field(
		'display_location',
		__( 'Where will the notification bar display?', 'wcmia-notifications' ),
	...
	add_settings_field(
		'display_sticky',
		__( 'Will the notificaton bar be sticky?', 'wcmia-notifications' ),
		'radio_input_callback',
		'wnb_general_settings',
		'general_section',
		array(
...
function general_settings_callback() {
	esc_html_e( 'WCMIA Notification Settings', 'wcmia-notifications' );
}
...
function wnb_display_notification_bar() {
	if ( null !== get_option( 'wcmia_general_settings' ) ) {
		$options = get_option( 'wcmia_general_settings' );
		?>
		<div class="wcmia-notification-bar <?php echo get_theme_mod( 'display_location' ); ?> <?php echo get_theme_mod( 'display_sticky' ); ?>">
			<div class="wcmia-notification-text"><?php echo get_theme_mod( 'notification_text' ); ?></div>
		</div>
		<?php
	}
}
...
/**
 * Adds Customizer Features
 */
add_action( 'customize_register', 'wcmia_customize_register' );
function wcmia_customize_register( WP_Customize_Manager $wp_customize ) {
	$wp_customize->add_section(
		'wcmia_notification_bar',
		array(
			'title' => __( 'Notification Bar', 'wcmia-notifications' ),
		)
	);
	...
	$wp_customize->add_control(
		...
		array(
			'type'        => 'radio',
			'section'     => 'wcmia_notification_bar', // Add a default or your own section
			'label'       => __( 'Display Location' ),
			'description' => __( 'Choose where the notification bar is displayed' ),
			'choices'     => array(
				'display_none'   => __( 'Do not display notification bar', 'wcmia-notifications' ),
				'display_top'    => __( 'Display notification bar on the top of the site', 'wcmia-notifications' ),
				'display_bottom' => __( 'Display notification bar on the bottom of the site', 'wcmia-notifications' ),
			),
		)
	);
	$wp_customize->add_setting(
	...
	$wp_customize->add_control(
		...
		array(
			...
			'choices'     => array(
				'display_sticky'   => __( 'Make the notification bar sticky', 'wcmia-notifications' ),
				'display_relative' => __( 'Do not make the notification bar sticky', 'wcmia-notifications' ),
			),
		)
	);
	$wp_customize->add_setting(
	...
	$wp_customize->add_control(
		...
		array(
			'type'        => 'textarea',
			'section'     => 'wcmia_notification_bar',
